...
datatable(df)
head(df.participation, 20) %>% droplevels() %$%
...
qplot(x = n, data = df.participation, geom="histogram")

ggplot(df.participation, aes(x = factor(1), fill = factor(n))) +
 geom_bar(width = 1) +
  coord_polar(theta = "y")
